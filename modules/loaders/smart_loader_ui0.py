...
def _render_edit_flow(file_type: str, cfg: dict):
    ...
    with st.expander("📥 Step 1 — Download Current Data", expanded=True):
        st.write(f"Download all current **{cfg['label']}** records from the database.")
        ...
        with col1:
            if st.button("⬇️ Download for Editing", key=f"dl_edit_{file_type}", type="primary"):
                _do_edit_download(file_type, cfg)
...
def _do_edit_download(file_type: str, cfg: dict):
    ...
    if submitted:
        ...
        try:
            ...
            if prod_rows:
                ...
                """, (prod_id, product_name.strip(), brand.strip(), 'PCS', float(gst_pct)))
        ...
        try:
            ...
            if stock_rows:
                ...
                """, (new_qty, float(cost_price), float(selling_price), float(mrp), prod_id, batch_key))
                ...
                """, (
                    ...
                    qty, float(cost_price), float(selling_price), float(mrp)
                ))
...
def _render_main_groups():
    ...
    add_hsn  = a3.text_input("HSN Code", placeholder="e.g. 90015000",             key="mg_new_hsn")
    add_desc = a4.text_input("Note", placeholder="optional",                      key="mg_new_desc")

    if st.button("➕ Add Group", key="mg_add_btn", type="primary"):
