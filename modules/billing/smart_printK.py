        ...
        if _partner_eye:
            for _pl in lines:
                if (str(_pl.get("eye_side") or "").upper() == _partner_eye
                        ...
                        and abs(normalize_to_pcs_price(float(_pl.get("unit_price") or 0), _pl) - up_raw) < 0.01):
                    _partner = _pl
                    break
    ...
    if show_bank and any([shop.get("bank_name"), shop.get("bank_account"), shop.get("shop_upi_id")]):
        ...
        if shop_upi:
            try:
                ...
                _upi_str = "upi://pay?pa={}&pn={}&cu=INR".format(
                    shop_upi,
                    shop_name.replace(" ", "%20")
                )
                _qr = qrcode.QRCode(
                    version=None,
...
""" if include_script else ""
    ...
    if _is_a4:
        _CSS_USE = (_CSS
            ...
            .replace('.inv-wrap{font-family:', '.inv-wrap{width:186mm;font-size:0.82rem;font-family:')
...
def render_smart_invoice(invoice_no: str, return_html: bool = False, doc_type_override: str = None, show_powers_override: bool = None, show_bank_override: bool = None):
    ...
    # ── Print options ──────────────────────────────────────────────────────
    if not return_html:
        import uuid as _spi_uuid
        _spi_sfx = invoice_no.replace("/","_") + "_" + _spi_uuid.uuid4().hex[:6]
    ...
    html = build_invoice_html(
        ...
        paper_size=paper_size if not return_html else 'A5',
    )
    if return_html:
        return html
...
def build_challan_html(
    ...
    if shop_upi:
        try:
            ...
            _upi_str = "upi://pay?pa={}&pn={}&cu=INR".format(shop_upi, shop_name.replace(" ", "%20"))
...
def render_smart_challan(challan_no: str, return_html: bool = False, doc_type_override: str = None, show_powers_override: bool = None):
    ...
    if not return_html:
        ...
        show_powers = c2.checkbox("Show Lens Powers",
                                  ...
                                  key="spc_pw_" + re.sub(r'[^a-zA-Z0-9_]', '_', str(challan_no)))
