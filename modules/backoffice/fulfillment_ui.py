...
def render_supplier_order_panel(ctx: BackofficeContext) -> None:
    """
    Supplier procurement section — VENDOR lines only.
    EXTERNAL_LAB lines are handled by the External Lab tab in supplier_panel.py.
    This prevents spurious CLIENT_ORDER POs being created for lab-routed lines.
    """
    ...
    if ext_lab_lines:
        st.info(
            ...
            "manage them in the **Supplier Orders → External Lab** tab below."
        )
...
def check_assignment_guard(ctx: BackofficeContext) -> bool:
    ...
    For all-STOCK orders (e.g. COUNTER_SALE), auto-confirms — no UI gate.
    ...
    # Pass all_lines so is_assignment_confirmed can auto-confirm STOCK-only orders
    if not is_assignment_confirmed(ctx.session, all_lines=ctx.all_lines):
        st.warning(
