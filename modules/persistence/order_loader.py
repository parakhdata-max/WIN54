    ...
    try:
        ...
        try:
            ...
            SELECT
                ...
                p.allow_loose,                                           -- 28
                p.product_code,                                          -- 29
                p.category                        AS product_category,   -- 34
        ...
        for row in cur.fetchall():
            ...
            # normalize_eye_side maps O/OTHER/NULL → B so all downstream
            # grouping, routing, and stock logic receives a canonical value.
            # This also silently fixes any dirty rows already in the DB.
            eye_side = normalize_eye_side(str(row[6]).strip() if row[6] else None)
            ...
            line = {
                ...
                # product.category — the specific lens type (e.g. "Kryptok", "Single Vision")
                # Used by job_card_surfacing to look up the correct blank inventory category.
                # row[34] only exists when the main SELECT ran (not the pricing-fallback SELECT).
                "category":            (row[34] if len(row) > 34 else None) or "",
                ...
                # ── Unpack stock/frame/supplier fields from lens_params ──────────
                # These are stored in lens_params JSONB by retail/wholesale punching.
                # Unpacking them here means assignment_panel, batch_manager, and all
                # downstream UI can use line.get("batch_no") directly — no dict diving.
                "batch_no":        (lens_params.get("batch_no") or "") if isinstance(lens_params, dict) else "",
                "batch_status":    (lens_params.get("batch_status") or "") if isinstance(lens_params, dict) else "",
                "lens_item_type":  (lens_params.get("lens_item_type") or "") if isinstance(lens_params, dict) else "",
                "colour_mix":      (lens_params.get("colour_mix") or "") if isinstance(lens_params, dict) else "",
                "frame_group":     (lens_params.get("frame_group") or "") if isinstance(lens_params, dict) else "",
                # supplier_id: prefer top-level DB column (set on save) then lens_params fallback
                "supplier_id":     (lens_params.get("supplier_id") or "") if isinstance(lens_params, dict) else "",
                "supplier_name":   (lens_params.get("supplier_name") or "") if isinstance(lens_params, dict) else "",
                # batch_allocation: list of {batch_no, allocated_qty, selling_price} dicts
                "batch_allocation": (lens_params.get("batch_allocation") or []) if isinstance(lens_params, dict) else [],
            ...
            elif _route == "EXTERNAL_LAB":
                order.setdefault("external_lab_lines", []).append(line)
            elif _route == "VENDOR":
                order["lab_order_lines"].append(line)
            else:
                # Unassigned — default to stock_lines (frames, accessories)
                _alloc_qty = int(line.get("allocated_qty") or 0)
                if _alloc_qty > 0 or str(line.get("batch_status","")).upper() == "ALLOCATED":
                    order["stock_lines"].append(line)
                else:
                    order["stock_lines"].append(line)
